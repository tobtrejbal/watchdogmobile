...
public class FlatBufferBuilder {
    ...
    boolean force_defaults = false; // False omits default values from the serialized SampleDAO.
    ...
    */
    ...
    /**
     * Doubles the size of the backing {@link ByteBuffer} and copies the old SampleDAO towards the
     * end of the new buffer (since we build the buffer backwards).
     *
     * @param bb The current buffer with the existing SampleDAO.
     * @return A new byte buffer with the old SampleDAO copied copied to it.  The SampleDAO is
    ...
    * the string SampleDAO follows it directly.  If all you need to do is alignment, `additional_bytes`
    * will be 0.
    *
    ...
    */
     ...
     * called `finish()`. The actual SampleDAO starts at the ByteBuffer's current position,
   ...
   /**
    * The FlatBuffer SampleDAO doesn't start at offset 0 in the {@link ByteBuffer}, but
    * now the {@code ByteBuffer}'s position is set to that location upon {@link #finish(int)}.
    *
    * @return The {@link ByteBuffer#position() position} the SampleDAO starts in {@link #dataBuffer()}
   ...
   /**
    * A utility function to copy and return the ByteBuffer SampleDAO from `start` to
    * `start` + `length` as a `byte[]`.
    *
    * @param start Start copying at this offset.
    * @param length How many bytes to copy.
    * @return A range copy of the {@link #dataBuffer() SampleDAO buffer}.
   ...
   /**
    * A utility function to copy and return the ByteBuffer SampleDAO as a `byte[]`.
    *
    * @return A full copy of the {@link #dataBuffer() SampleDAO buffer}.
    */
