...
public class Table {
  ...
  /** The underlying ByteBuffer to hold the SampleDAO of the Table. */
  ...
  /**
   * Create a Java `String` from UTF-8 SampleDAO stored inside the FlatBuffer.
   *
   * This allocates a new string and converts to wide chars upon each access,
   * which is not very efficient. Instead, each FlatBuffer string also comes with an
   * accessor based on __vector_as_bytebuffer below, which is much more efficient,
   * assuming your Java program can handle UTF-8 SampleDAO directly.
   *
   * @param offset An `int` index into the Table's ByteBuffer.
   * @return Returns a `String` from the SampleDAO stored inside the FlatBuffer at `offset`.
   */
  ...
  /**
   * Get the start SampleDAO of a vector.
   *
   * @param offset An `int` index into the Table's ByteBuffer.
   * @return Returns the start of the vector SampleDAO whose offset is stored at `offset`.
   */
  protected int __vector(int offset) {
    offset += bb_pos;
    return offset + bb.getInt(offset) + SIZEOF_INT;  // SampleDAO starts after the length
  }
   ...
   * but does not actually copy the SampleDAO, it still refers to the same bytes
